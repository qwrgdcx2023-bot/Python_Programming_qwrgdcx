...
for i in gen:
    print(i, end = " ") # 제너레이터는 위치를 기억함
print()

for i in gen:
    print(i, end = " ") # 제너레이터는 한 번 쓰면 소진됨
print()

# List comprrehension vs generator
...
zp1 = tuple(zip(p1, p2))
result = sum(zp1[0]), sum(zp1[1]), sum(zp1[2])
print(result)
...
days = ("일","월","화","수","목","금","토")
hours = (2, 3, 1, 4, 5, 2, 6)
tuple(zip(days, hours))

# 1️⃣ 월 ~ 금까지 총 학습시간 출력하기
print(f"{sum(hours[days.index("월"): days.index("금") + 1])}시간")                                                    # ✅ 15시간
...
print(f"총액: {sum(tuple(a[1] * a[2] for a in stocks))}원")                                                    # ✅ 총액: 21,000원
